...
def farward_loss(args, input_batch, model, robot, device, device_id, train=True, **kwargs):
    curr_epoch = kwargs.get("epoch", None)
    val_dsname = kwargs.get("dsname", None)
    if train:
        model.train()
    else:
        model.eval() 
    ...
    #? gt_rot: 3 * 3 matrix for rotation, gt_rot  shape: torch.Size([48, 3, 3])
    #? gt_trans shape: torch.Size([48, 3])  T(4*4) [:3, 3] is the translation vector 
    ...
    if args.use_extended_bbox:
        fx, fy = root_K[:,0,0], root_K[:,1,1]
    #* f_x f_y 是相机内参的(0, 0)处和(1, 1)处element，或者选K_original的或者是root_K的
    ...
    #TODO: 这个k_values 对应原论文(1)公式 d_c =\sqrt{\frac{f_x f_y A_{real}}{A_{bbox}}}
    ...
    if args.multi_kp:
        pred_pose, pred_rot, pred_trans, pred_root_uv, pred_root_depth, pred_depths, \
            pred_uvd, pred_keypoints3d_int, pred_keypoints3d_fk, pred_logit_3dkp, gt_indicies_3dkp = model(reg_images, root_images, k_values, K=other_K, gt_keypoints3d=gt_keypoints3d, gt_keypoints2d=gt_keypoints2d, valid_mask_crop=valid_mask_crop, TCO_matirx=TCO, train=train, epoch=curr_epoch, val_dsname=val_dsname)
    else:
    ...
    #! Add all uv supervise(not only with root point)
    pred_uvs = (pred_uvd[..., 0:2] + 0.5) * args.other_image_size
    ...
    assert(pred_uvs.shape == gt_keypoints2d.shape), f"{pred_uvs.shape}, {gt_keypoints2d.shape}"
    # metrics during validation
    if not train:
        #* compute metrics batch 的计算：
        # print(f"3dkp_int: {pred_keypoints3d_int[0]}\n3dkp_gt{gt_keypoints3d[0]}\n")
    ...
    if args.uvs_loss_func == "smoothl1":
        loss_uvs = SmoothL1Loss(pred_uvs/args.image_size , gt_keypoints2d/args.image_size)
    elif args.uvs_loss_func == "l1":
        loss_uvs = L1Loss(pred_uvs/args.image_size , gt_keypoints2d/args.image_size)
    elif args.uvs_loss_func == "mse":
        loss_uvs = MSELoss(pred_uvs/args.image_size , gt_keypoints2d/args.image_size)
    # elif args.uvs_loss_func == "l2norm":
    #     error_uvs = torch.norm((pred_uvs - gt_keypoints2d)/args.image_size, dim = 1)
    #     # error_root_uv = cast(error_root_uv,device)
    #     # loss_uv = torch.mean(error_root_uv)
    #     error_uvs = cast(error_uvs,device) * valid_mask_crop[:,args.reference_keypoint_id]
    #     loss_uvs = torch.sum(error_uvs) / torch.sum(valid_mask_crop[:,args.reference_keypoint_id] != 0)
    else:
        raise(NotImplementedError)
    ...
    vq_token_criteria = nn.CrossEntropyLoss()
    if pred_logit_3dkp is not None and gt_indicies_3dkp is not None:
        vq_token_loss = vq_token_criteria(pred_logit_3dkp, gt_indicies_3dkp)
    else:
        vq_token_loss = cast(torch.tensor([0.]), device)
    ...
    loss = args.pose_loss_weight * loss_pose + args.rot_loss_weight * loss_rot + \
            ...
            args.uvs_loss_weight * loss_uvs + vq_token_loss
    #? 所有Loss：
    #? loss_pose, loss_rot, loss_uv, loss_depth, loss_trans, 
    #? loss_error2d, loss_error3d, loss_error2d_int, loss_error3d_int, 
    #? loss_error3d_align
    ...
    loss_dict = {   
        ...
        "loss_uv": loss_uv, "loss_uvs": loss_uvs, "loss_depth": loss_depth, "loss_trans": loss_trans,
        "loss_error2d": loss_error2d, "loss_error3d": loss_error3d,
        "loss_error2d_int": loss_error2d_int, "loss_error3d_int": loss_error3d_int,
        "loss_error3d_align": loss_error3d_align, "loss_token": vq_token_loss
    }
    if args.multi_kp:
        loss_dict["loss_depth_multi"] = loss_depth_multi
...
def validate(args, epoch, dsname, loader, model, robot, writer, device, device_id):
    ...
    losses_pose, losses_rot, losses_trans, losses_uv, losses_depth, losses_error2d, losses_error3d, losses_error2d_int, losses_error3d_int, losses_error3d_align, rotation_diff, losses_vq = \
        AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter(), AverageValueMeter()
    # losses_token = AverageValueMeter()
    losses_uvs = AverageValueMeter()
    alldis = defaultdict(list)     #! dict_keys(['dis3d', 'dis2d', 'jointerror'])
    alldis_int = defaultdict(list) #! dict_keys(['dis3d', 'dis2d'])
    ...
    metric_uvs_dis2d = [AverageValueMeter() for i in range(len(robot.link_names))]
    metric_l1joint = [AverageValueMeter() for i in range(robot.dof)]
    
    with torch.no_grad():
        for idx, sample in enumerate(tqdm(loader, dynamic_ncols=True, desc=f"Validating {dsname}: {epoch}")):
            vloss, loss_dict, metric_dict = farward_loss(args=args, input_batch=sample, model=model, robot=robot, device=device, device_id=device_id, train=False, dsname=dsname)
            #* {'image_dis3d_avg': (48,), 'image_dis2d_avg': (48,), 'batch_dis2d_avg': (7,), 'root_depth_error': (48,), 'image_dis3d_avg_int': (48,), 'image_dis2d_avg_int': (48,), 'batch_dis2d_avg_int': (7,), 'root_depth_error_int': (48,), 'rotation_diff': torch.Size([])}
            ...
            alldis["dis3d"].extend(metric_dict["image_dis3d_avg"]) #& 这里 image_xxx_avg，因为有多个joints，返回的是一个batch 每个样本里，7个point 误差的平均值
            alldis["dis2d"].extend(metric_dict["image_dis2d_avg"]) #& 3d 表示距离(mm)，而2d 表示像素坐标距离
            ...
            # for id in range(len(robot.link_names)):
            #     metric_uvs_dis2d[id].add(metric_dict["batch_uvs_avg_error"][id])
    ...
    # writer.add_scalar('Val/token_loss'+ds, losses_token.mean , epoch)
